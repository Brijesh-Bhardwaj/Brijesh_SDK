...
class BSAmazonAuthenticator: BSBaseAuthenticator {
    ...
    override func onPageFinish(url: String) throws {
        print("### didFinish")
        if (url.contains(AmazonURL.signIn)) {
            self.injectAuthErrorVerificationJS()
            print("### urlString sign in")
        } else if (url.contains("/gp/your-account/order-history")) {
            print("### urlString OrderHistory", url)
            self.listener?.onAuthenticationSuccess()
        } else {
            print("### urlString Other")
