...
class InstacartOrderScrapper {
    ...
    private var instAuth: ConnectInstacartAccountVC!
    
    private static var instance: InstacartOrderScrapper!
    
    public static let shared: InstacartOrderScrapper = {
        if instance == nil {
            instance = InstacartOrderScrapper()
        }
        return instance
    }()
    ...
    func disconnectAccount(account: Account, accountDisconnectedListener: AccountDisconnectedListener, orderSource: String) {
        _ = AmazonService.updateStatus(platformId: account.userID, status: AccountState.ConnectedAndDisconnected.rawValue, message: AppConstants.msgDisconnected, orderStatus: OrderStatus.None.rawValue, orderSource: OrderSource.Instacart.value) { response, error in
            if response != nil {
    ...
    private func shouldStartScrapping(completion: @escaping(Bool) -> Void) {
        // ConfigObject to
        ConfigManager.shared.getConfigurations(orderSource: .Instacart) { (configurations, error) in
